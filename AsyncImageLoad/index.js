...
async function preloadImages(imageUrlsArr) {
    ...
    try {
        const promises = images.map(image=>getImagePromise(image))
        const result = await Promise.all(promises)
        console.log('All images loaded successfully!')
        uploadContainer.style.display="none"
        result.forEach(image=>imgContainer.appendChild(image))
    } catch (err){
        imgContainer.innerHTML = 'An error occured ' + err
        console.log('An error occured preloading images' + err)
    }
